...
AIReading::GetReading(const std::vector<CardInfo>& cards)
{
	...
	if (!Config::IsAPIKeySet()) {
		BString message = "OpenAI API key not set. Please set the OPENAI_API_KEY environment variable.\n\n";
		message += "Card spread: ";
	...
	BString prompt = "Provide a tarot card reading for the following three cards drawn in a simple spread: ";
	...
	bool parsingSuccessful = reader->parse(response.c_str(), response.c_str() + response.size(), 
	                                       &jsonResponse, &errs);
